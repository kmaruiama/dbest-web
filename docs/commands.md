...
make build                         # create build/libs/dbest-0.1.0-SNAPSHOT.jar
...
Install client dependencies and Chromium once before running browser tests:
...
Before invoking npm directly on a clean clone, install dependencies with
`./gradlew :app:client:npmInstall` from the repository root.
...
set. Start `./gradlew :app:server:run` separately for API-only development.

## Gradle commands

```bash
./gradlew check                        # server and engine checks; no Node.js required
./gradlew :run                         # build the client and run the integrated application
./gradlew :bundledJar                  # build the self-contained application JAR
./gradlew :installDist                 # integrated app with separate dependency JARs
./gradlew :signBundledJar              # sign the application JAR using DBEST_SIGNING_* variables
./gradlew :app:client:npmInstall       # install locked client dependencies without building
./gradlew :app:client:build            # install dependencies and build only the client
./gradlew :app:server:build            # build and test only the server and required engine artifacts
./gradlew :app:server:run              # run only the API server; pair with Vite
./gradlew :app:server:installDist      # API-only distribution used by E2E with Vite
